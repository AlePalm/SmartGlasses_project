...
void Setup_Values(void)
{
    ...
    uint8_t capdac1[2] = {16,0};
    uint8_t capdac2[2] = {48,0};
    uint8_t capdac3[2] = {80,0};
    uint8_t capdac4[2] = {112,0};
    
    uint8_t gain1[2] = {64,0};
    uint8_t gain2[2] = {64,0};
    uint8_t gain3[2] = {64,0};
    uint8_t gain4[2] = {64,0};
    
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_CONF_MEAS1, 2, capdac1);
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_CONF_MEAS2, 2, capdac2);
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_CONF_MEAS3, 2, capdac3);
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_CONF_MEAS4, 2, capdac4);
    
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_GAIN_CAL_CIN1, 2, gain1);
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_GAIN_CAL_CIN2, 2, gain2);
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_GAIN_CAL_CIN3, 2, gain3);
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_GAIN_CAL_CIN4, 2, gain4);
    
    I2C_Peripheral_WriteRegisterMulti(FDC1004Q_I2C_ADDR, FDC1004Q_FDC_CONF, 2, write);
}
...
uint8_t FDC_SetRawOffsetCalibrationSingle_1(uint8_t channel, int16_t offset)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    float offset_f = fixed_to_float_signed(offset, FIXED_POINT_FRACTIONAL_BITS_OFFSET);
    if ( (offset_f < 0) || (offset_f > 16))
        return FDC_CONF_ERR;
    uint8_t temp[2] = {offset >> 8, offset & 0xFF};
    return FDC_WriteRegister(FDC1004Q_OFFSET_CAL_CIN1, temp);
}
...
uint8_t FDC_ReadOffsetCalibrationSingle(uint8_t channel, float* offset)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    uint8_t temp[2];
    uint8_t error = FDC_ReadRegister(FDC1004Q_OFFSET_CAL_CIN1, temp);
    if (error == FDC_OK)
    {
        int16_t reg = temp[0] << 8 | temp[1];
        float off = (float) (reg);
...
uint8_t FDC_ReadRawOffsetCalibrationSingle(uint8_t channel, int16_t* offset)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    uint8_t temp[2];
    uint8_t error = FDC_ReadRegisterSingle(FDC1004Q_OFFSET_CAL_CIN1, temp);
    if (error == FDC_OK)
    {
        *offset = temp[0] << 8 | temp[1];
    }
...
uint8_t FDC_SetGainCalibrationSingle(uint8_t channel, float gain)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    if ( (gain < 0) || (gain > 4))
    ...
    uint16_t gain_u16 = float_to_fixed_unsigned(gain, FIXED_POINT_FRACTIONAL_BITS_GAIN);
    uint8_t temp[2] = {gain_u16 >> 8, gain_u16 & 0xFF};
    return FDC_WriteRegister(FDC1004Q_GAIN_CAL_CIN1, temp);
}
...
uint8_t FDC_SetRawGainCalibrationSingle_1(uint8_t channel, uint16_t gain)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    float gain_f = fixed_to_float_unsigned(gain, FIXED_POINT_FRACTIONAL_BITS_GAIN);
    if ( (gain_f < 0) || (gain_f > 4))
        return FDC_CONF_ERR;
    uint8_t temp[2] = {gain >> 8, gain & 0xFF};
    return FDC_WriteRegisterSingle(FDC1004Q_GAIN_CAL_CIN1, temp);
}

uint8_t FDC_SetRawGainCalibrationSingle_2(uint8_t channel, uint16_t gain)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    float gain_f = fixed_to_float_unsigned(gain, FIXED_POINT_FRACTIONAL_BITS_GAIN);
    if ( (gain_f < 0) || (gain_f > 4))
        return FDC_CONF_ERR;
    uint8_t temp[2] = {gain >> 8, gain & 0xFF};
    return FDC_WriteRegisterSingle(FDC1004Q_GAIN_CAL_CIN2, temp);
}
...
uint8_t FDC_ReadGainCalibrationSingle(uint8_t channel, float* gain)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    uint8_t temp[2];
    uint8_t error = FDC_ReadRegisterSingle(FDC1004Q_GAIN_CAL_CIN1, temp);
    if (error == NO_ERROR)
    {
        *gain = fixed_to_float_unsigned(temp[0] << 8 | temp[1], FIXED_POINT_FRACTIONAL_BITS_GAIN);
    }
...
uint8_t FDC_ReadRawGainCalibrationSingle(uint8_t channel, uint16_t* gain)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    uint8_t temp[2];
...
uint8_t FDC_ReadRawMeasurementSingle_1(uint8_t channel, uint32_t* capacitance)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    uint8_t temp[2];
    uint8_t error = FDC_ReadRegisterSingle(FDC1004Q_MEAS1_MSB, temp);
    if (error == FDC_OK)
    {
        *capacitance = (temp[0] << 24) | (temp[1] << 16);
        error = FDC_ReadRegisterSingle(FDC1004Q_MEAS1_LSB, temp);
        if (error == FDC_OK)
        {
            *capacitance |=  temp[0] << 8 | temp[1];
        }
...
uint8_t FDC_ReadRawMeasurementSingle_2(uint8_t channel, uint32_t* capacitance)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    uint8_t temp[2];
    uint8_t error = FDC_ReadRegisterSingle(FDC1004Q_MEAS2_MSB, temp);
    if (error == FDC_OK)
    {
        *capacitance = (temp[0] << 24) | (temp[1] << 16);
        error = FDC_ReadRegisterSingle(FDC1004Q_MEAS2_LSB, temp);
        if (error == FDC_OK)
        {
            *capacitance |=  temp[0] << 8 | temp[1];
        }
...
uint8_t FDC_ReadRawCapdacSetting_1(uint8_t channel, uint8_t* capdac)
{
    if (channel > FDC_CH_4)
        return FDC_CONF_ERR;
    // Read current capdac setting
    uint8_t temp[2];
    uint8_t error = FDC_ReadRegister(FDC1004Q_CONF_MEAS1, temp);
    if ( error == FDC_OK)
    {
            // Read current capdac
            uint16_t temp16 = temp[0] << 8 | temp[1];
...
uint8_t FDC_ReadRegisterSingle(uint8_t reg_addr, uint8_t* data)
{
    uint8_t error = I2C_Peripheral_ReadRegister(FDC1004Q_I2C_ADDR, reg_addr, data);
    if (error == NO_ERROR)
    {
        return FDC_OK;
    }
...
uint8_t FDC_WriteRegisterSingle(uint8_t reg_addr, uint8_t* data)
{
    uint8_t error = I2C_Peripheral_WriteRegister(FDC1004Q_I2C_ADDR, reg_addr, *data);
    if (error == NO_ERROR)
    {
        return FDC_OK;
    }
